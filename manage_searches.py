...
def split_filter_arg(filter_arg, sep=';'):
...
def delete_saved_search(session, search_name=None, search_id=None):
    if search_id:
        all_searches = get_all_searches(session, key='id')
        if search_id in all_searches.keys():
            delete_url = "{}/{}".format(SEARCH_URL, search_id)
    elif search_name:
        all_searches = get_all_searches(session, key='name')
        delete_url = "{}/{}".format(SEARCH_URL, all_searches["name"]["id"])
